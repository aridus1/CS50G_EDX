...
function Level:init()
    ...
    -- Adv: cannot split after collision, so keep record of it
    ...
    --Adv: table for extra player aliens to search through
    self.players = {}
    ...
    -- flag tot check if player is fired and has split
    self.hasSplit = false
end
...
function Level:update(dt)
    ...
    for k, player in pairs(self.players) do
        ...
        -- Adv: also checking for each player to have stopped rolling
    ...
    --Adv: replace launch marker if original alien stopped moving
    if self.launchMarker.launched then
        local xPos, yPos = self.launchMarker.alien.body:getPosition()
        local xVel, yVel = self.launchMarker.alien.body:getLinearVelocity()
        
        -- if we fired our alien to the left or it's almost done rolling, respawn
        if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) and #self.players == 0 then
            self.launchMarker.alien.body:destroy()
            self.launchMarker = AlienLaunchMarker(self.world)

            --Adv: reset the ability to split
            self.hasSplit = false
            self.playerCollided = false
    ...
    -- Adv: allows skipping of victory scene (really only useful is player is still rolling)
...
function Level:render()
    ...
    --if self.launchMaker.alien ~= nil then
